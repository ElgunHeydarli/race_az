...
const CACHE_KEY = 'race_hero_cache';

function getCachedHero() {
  try {
...
function setCachedHero(race: Competition) {
  try {
    localStorage.setItem(CACHE_KEY, JSON.stringify({
      name: race.name,
      text: race.text,
      slug: race.slug,
      bottom_image: race.bottom_image,
      mobile_image: race.mobile_image,
      competition_start_date: race.competition_start_date,
    }));
  } catch {}
}
...
const HeaderSection = () => {
  ...
  const [nearestRace, setNearestRace] = useState<Competition | null>(cached);
  const [backgroundImage, setBackgroundImage] = useState<string>(cached?.bottom_image || '');
  const [mobileImage, setMobileImage] = React.useState<string>(cached?.mobile_image || '');
  ...
  // Preload background image — fade in when ready
  ...
  return (
    <header
      ...
      <motion.div
        ...
        <div className="md:py-[40px] py-[30px]">
          <Timer />
        </div>
